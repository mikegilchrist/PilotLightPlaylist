...
# Usage: python pilotlight_scraper.py [--week_start N] [--week_stop M] [--verbose] [--filename FILE]
# Input:
#   --week_start: Weeks from today to start (default: 0)
#   --week_stop:  Weeks from today to stop (default: no limit)
#   --filename:   Optional output filename (default: upcoming-artists_<date>_<time>.csv)
# Output: Writes events to "upcoming-artists_<YYYY-MM-DD_HH:MM>.csv"
...
def scrape_upcoming_artists(output_path=None, week_start=0, week_stop=-1, verbose=False):
    ...
    def parse_event(p_tag):
        ...
        m = re.search(r"(\w+day)\s+([A-Za-z]+)\s+(\d+).*?\$(FREE|\d+)", text, flags=re.IGNORECASE)
        if not m:
            return None
...
if __name__ == "__main__":
    ...
    parser.add_argument("--filename", type=str, help="Output CSV filename")
